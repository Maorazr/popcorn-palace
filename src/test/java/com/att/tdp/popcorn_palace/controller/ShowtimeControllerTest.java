...
class ShowtimeControllerTest {
  ...
  @Mock private ShowtimeService service;

  @InjectMocks private ShowtimeController controller;
  ...
  @Test
  @DisplayName("get showtime by id and return status 200 OK")
  ...
  @Test
  @DisplayName("fail to get showtime by id throws ShowtimeNotFoundException")
  void testGetShowtimeById_NotFound() {
    when(service.getShowtimeById(999L)).thenThrow(new ShowtimeNotFoundException(999L));

    ShowtimeNotFoundException ex =
        assertThrows(ShowtimeNotFoundException.class, () -> controller.getShowtimeById(999L));
    assertEquals("Showtime not found with ID = 999", ex.getMessage());
    verify(service, times(1)).getShowtimeById(999L);
  }

  @Test
  @DisplayName("adds a showtime and returns status 200 OK")
  ...
  @Test
  @DisplayName("fail to add showtime throws OverlappingShowtimeException")
  void testAddShowtime_Overlapping() {
    ...
    OverlappingShowtimeException ex =
        assertThrows(OverlappingShowtimeException.class, () -> controller.addShowtime(dto));

    assertEquals(
        "Cannot create showtime; overlapping schedule in theater 'Cinema 2'.", ex.getMessage());

    verify(service, times(1)).addShowtime(any(ShowtimeDto.class));
  }

  @Test
  @DisplayName("fails to add showtime throws BadRequestException")
  ...
  @Test
  @DisplayName("update showtime and return status 200 OK")
  ...
  @Test
  @DisplayName("fails to update showtime throws ShowtimeNotFoundException")
  void testUpdateShowtime_NotFound() {
    ShowtimeDto dto = ShowtimeDto.builder().build();

    doThrow(new ShowtimeNotFoundException(999L))
        .when(service)
        .updateShowtime(eq(999L), any(ShowtimeDto.class));

    ShowtimeNotFoundException ex =
        assertThrows(ShowtimeNotFoundException.class, () -> controller.updateShowtime(999L, dto));
  ...
  @Test
  @DisplayName("delete showtime and return status 200 OK")
  ...
  @Test
  @DisplayName("fails to delete showtime throws ShowtimeNotFoundException")
  void testDeleteShowtime_NotFound() {
    doThrow(new ShowtimeNotFoundException(888L)).when(service).deleteShowtime(888L);
    ShowtimeNotFoundException ex =
        assertThrows(ShowtimeNotFoundException.class, () -> controller.deleteShowtime(888L));
